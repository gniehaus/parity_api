...
def clean_nanos_chain(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Only use real two-sided quotes.
...
def build_weekly_income_outcome(
    chain: pd.DataFrame,
    etf_price: float,
    nanos_price: float,
    target_income_pct: float,
):
    ...
    actual_income_pct = float(best["income_pct"])
    ...
    return {
        "product_key": "weekly_income",
        "title": f"{target_income_pct * 100:.1f}% Weekly Income",
        "subtitle": "Earn income while keeping S&P 500 exposure.",
        ...
        "income_target_met": actual_income_pct >= target_income_pct,

        "short_call_symbol": best.get("symbol"),
        "short_call_strike": float(best["strike"]),
        "call_bid": float(best["bid"]),
        "call_ask": float(best["ask"]),
        "call_mid": float(best["mid"]),
        "call_credit_dollars": float(best["mid"]),

        "income_pct": actual_income_pct,
        "income_dollars": float(best["mid"]),
        "price_upside_pct": price_upside_pct,
        "max_upside_pct": max_upside_pct,
        "breakeven_pct": breakeven_pct,

        "downside_note": "Downside risk remains if the market falls.",
        ...
        "liquidity": {
            ...
            "bid_ask_spread": float(best["ask"] - best["bid"]),
            "spread_pct": float(best["spread_pct"]),
        },

        "display": {
            "primary_metric": f"{actual_income_pct * 100:.1f}%",
            "primary_label": "Weekly income",
            "secondary_metric": f"{max_upside_pct * 100:.1f}%",
            "secondary_label": "Total max upside",
            "estimated_income_label": f"${float(best['mid']):.2f}",
            "explanation": (
                "This outcome generates income by giving up upside above the selected level. "
                "Downside risk remains if the market falls."
            ),
        },
    }
...
def build_products_for_expiration(
    ...
    for target in income_targets:
        outcome = build_weekly_income_outcome(
            chain=chain,
            etf_price=etf_price,
            nanos_price=nanos_price,
            target_income_pct=target,
        )

        key = f"income_{target * 100:.1f}".replace(".", "_")

        products[key] = outcome
...
def build_weekly_outcomes_payload(
    ...
    if expirations_df.empty:
        return {
            ...
            "title": "Weekly Income Outcomes",
            "subtitle": "Small-dollar S&P 500 income outcomes.",
    ...
    payload = {
        ...
        "assumptions": {
            ...
            "nanos_contracts": 1,
            "note": (
                "Displayed outcomes use the option reference level for payoff math. "
                "ETF price is used as the user-facing strategy size reference."
            ),
        },
        "quote_filter": {
