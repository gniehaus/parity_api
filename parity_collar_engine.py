...
# # ============================================================
# # 3. COLLAR ENGINE WITH DIVIDENDS
# # ============================================================
...
#     Dividend treatment:
#         Expected dividends are added to terminal outcomes.

#         expected_dividend_dollars =
#             notional * assumed_dividend_yield * (dte / 365.25)

#     Important:
#         Dividends are not treated as upfront cash.
#         They improve terminal floor/cap economics only.
#     """
...
#         expected_dividend_bps = expected_dividend_dollars / notional * 10000
...
#                     market_floor_return = put_strike / spot - 1
#                     market_cap_return = call_strike / spot - 1

#                     # Dividend-inclusive terminal values.
#                     floor_value = (
#                         put_strike * MULT
#                         - net_cost
#                         + expected_dividend_dollars
...
#                         "market_floor_return": market_floor_return,
#                         "market_cap_return": market_cap_return,
...
#                     # Dividend-inclusive protected zone.
#                     protected_zone_value = (
#                         short_put_strike * MULT
#                         + max_buffer_value
#                         - net_cost
...
#     for col in pct_cols:
#         if col in view.columns:
#             if col == "expected_dividend_bps":
#                 view[col + "_display"] = view[col]
#             else:
#                 view[col + "_pct"] = view[col] * 100
...
#         "assumed_dividend_yield_pct",
#         "expected_dividend_dollars",
#         "expected_dividend_bps_display",
...
#         type=float,
#         default=0.012,
#         help="Assumed annual dividend yield as decimal. Example: 0.012 for 1.2 percent.",
#     )
...
def clean_chain(df, ticker=None):
    ...
    chain = chain.dropna(subset=["expirDate", "dte", "strike", "spot"])
...
def select_single_expiry(chain, target_dte=365):
    """
    Select one expiration closest to the user's horizon.
    Both the collar and buffer use this same expiration.
    """
    ...
    selected = expiry_summary.iloc[0]
...
def liquidity_score(*rows):
    ...
    return np.log1p(total_volume) + np.log1p(total_oi), total_volume, total_oi
...
def build_zero_cost_dividend_floor_collar(
    ...
    Logic:
    1. Determine the lowest put strike that satisfies the user's max loss
       after expected dividends.
    2. Buy that put.
    3. Sell the highest call whose premium covers the put cost.
    4. Return the actual floor and cap.

    Terminal floor, assuming zero or net credit option package:
        floor_value = put_strike * 100 + expected_dividends - net_cost

    Where:
        net_cost = put_cost - call_credit

    We prefer:
        call_credit >= put_cost
    """
    ...
    # Required terminal floor value.
    target_floor_value = notional * (1 - max_loss_pct)

    # If net cost is zero, floor_value = put_strike * 100 + dividends.
    required_put_strike = (
    ...
    if valid_puts.empty:
        # Fallback: closest put below spot, even if it misses target.
        valid_puts = g[
            (g["strike"] < spot)
            & (g["putAskPrice"] > g["putBidPrice"])
            & (g["putMid"] > 0)
        ...
        # Lowest strike that still satisfies the floor.
    ...
    valid_calls["covers_put"] = valid_calls["call_credit_dollars"] >= put_cost

    calls_that_cover = valid_calls[valid_calls["covers_put"]].copy()

    if not calls_that_cover.empty:
        # Highest call strike that still fully funds the put.
        call = calls_that_cover.sort_values("strike", ascending=False).iloc[0]
        zero_cost_ok = True
    else:
        # Fallback: call with premium closest to put cost.
        valid_calls["premium_gap_abs"] = (
            valid_calls["call_credit_dollars"] - put_cost
        ).abs()
        call = valid_calls.sort_values("premium_gap_abs").iloc[0]
        zero_cost_ok = False
...
def build_zero_cost_target_cap_buffer(
    ...
    Logic:
    1. Find the call strike that gets closest to the user's target gain
       after expected dividends, assuming near-zero net option cost.
    2. Use that call premium to buy the widest possible put spread.
    3. Return the resulting buffer level.

    Important:
    Buffer is NOT a hard max loss.
    It protects a range of losses. Losses may continue beyond the buffer.
    """
    ...
    # If net cost is zero, cap_value = call_strike * 100 + dividends.
    ...
    # Pick call closest to target cap.
    ...
    # Long put should be at-the-money or nearest strike below spot.
    ...
    for _, short_put in short_puts.iterrows():
        ...
        # Require zero-cost or net-credit buffer if possible.
        zero_cost_ok = net_cost <= 0
        ...
        # Primary objective:
        #   maximize buffer subject to target cap call and zero cost.
        # Secondary:
        #   lower bid/ask drag and better liquidity.
        if zero_cost_ok:
            rank_score = (
                1000.0 * buffer_pct
                - 0.05 * bid_ask_drag_bps
            ...
            # Keep fallback candidates, but penalize debit.
            rank_score = (
                1000.0 * buffer_pct
                - 10.0 * abs(net_cost_bps)
                - 0.05 * bid_ask_drag_bps
                + liq_score
            )
    ...
    zero_cost_candidates = candidates[candidates["zero_cost_ok"]].copy()

    if not zero_cost_candidates.empty:
        best = zero_cost_candidates.sort_values(
            ["buffer_pct", "bid_ask_drag_bps", "total_oi"],
            ascending=[False, True, False],
        ).iloc[0]
    else:
        best = candidates.sort_values(
            ["rank_score", "buffer_pct", "bid_ask_drag_bps"],
    ...
    return {
        ...
        "zero_cost_ok": bool(best["zero_cost_ok"]),
...
# ============================================================
# 6. PRODUCT RECOMMENDATION PAYLOAD
# ============================================================
...
def build_defined_outcome_recommendations(
    ...
    1. Defined Floor
        ...
        - Then find the call that funds the put.

    2. Buffered Growth
        - Find the call first based on target top gain including dividends.
        - Then find the best buffer that can be funded by that call.
    ...
    expiry_chain, selected_expiry_summary, all_expiries = select_single_expiry(
        chain,
        target_dte=horizon,
    )
    ...
    payload = {
        ...
        "assumptions": {
            ...
            "cost_note": (
                "Products are constructed to target zero upfront option cost "
                "where possible."
            ),
        },
        "products": [
        ...
        "raw": {
            "expiry_summary": all_expiries.to_dict(orient="records"),
        },
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.005,
        help="Max loss target for collar as decimal. Example: 0.005 for 0.50 percent.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.08,
        help="Target gain for buffer call selection as decimal. Example: 0.08 for 8 percent.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Annual dividend yield assumption. Default is 0.01 for 1 percent.",
    )
