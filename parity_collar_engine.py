...
def has_positive_market(df, bid_col, ask_col):
    return (
        df[bid_col].notna()
        & df[ask_col].notna()
        & (df[bid_col] > 0)
        & (df[ask_col] > 0)
        & (df[ask_col] >= df[bid_col])
    )
...
def display_max_loss_pct(floor_return):
...
def leg_liquidity(row, leg_type):
    if row is None:
        return 0.0, 0.0

    if leg_type == "call":
        volume = float(row.get("callVolume", 0) or 0)
        oi = float(row.get("callOpenInterest", 0) or 0)
    else:
        volume = float(row.get("putVolume", 0) or 0)
        oi = float(row.get("putOpenInterest", 0) or 0)

    return volume, oi


def liquidity_score_legs(*legs):
    total_volume = 0.0
    total_oi = 0.0

    for row, leg_type in legs:
        volume, oi = leg_liquidity(row, leg_type)
        total_volume += volume
        total_oi += oi
...
def build_zero_cost_dividend_floor_collar(
    ...
    valid_puts = g[
        (g["strike"] < spot)
        & has_positive_market(g, "putBidPrice", "putAskPrice")
    ].copy()

    valid_calls = g[
        (g["strike"] > spot)
        & has_positive_market(g, "callBidPrice", "callAskPrice")
    ].copy()
    ...
    # Buy put at ask.
    put_costs = valid_puts["putAskPrice"].to_numpy(dtype=float) * MULT
    put_mids = valid_puts["putMid"].to_numpy(dtype=float) * MULT
    ...
    # Conservative executable pricing:
    # Sell call at bid.
    call_credits = valid_calls["callBidPrice"].to_numpy(dtype=float) * MULT
    call_mids = valid_calls["callMid"].to_numpy(dtype=float) * MULT
    ...
    net_mid_cost = put_mids[:, None] - call_mids[None, :]
    ...
    valid_mask = np.isfinite(floor_return) & np.isfinite(cap_return)
    floor_met_mask = valid_mask & (floor_return >= target_floor_return)
    near_zero = abs_net_cost_bps <= max_near_zero_bps

    if not np.any(floor_met_mask):
        selection_mask = valid_mask
        outside_tolerance = True
        exact_floor_match = False
    else:
        preferred_mask = floor_met_mask & near_zero

        if np.any(preferred_mask):
            selection_mask = preferred_mask
            outside_tolerance = False
        else:
            selection_mask = floor_met_mask
            outside_tolerance = True

        exact_floor_match = True

    execution_drag_dollars = net_cost - net_mid_cost
    execution_drag_bps = execution_drag_dollars / notional * 10000
    ...
    best_net_mid_cost = float(net_mid_cost[best_put_idx, best_call_idx])
    ...
    best_execution_drag_bps = float(execution_drag_bps[best_put_idx, best_call_idx])
    ...
    return make_json_safe({
        ...
        "put_cost_dollars": float(best_put["putAskPrice"]) * MULT,
        "put_mid_dollars": float(best_put["putMid"]) * MULT,
        "put_bid_per_share": float(best_put["putBidPrice"]),
        "put_ask_per_share": float(best_put["putAskPrice"]),

        "call_credit_dollars": float(best_call["callBidPrice"]) * MULT,
        "call_mid_dollars": float(best_call["callMid"]) * MULT,
        "call_bid_per_share": float(best_call["callBidPrice"]),
        "call_ask_per_share": float(best_call["callAskPrice"]),

        "net_cost_dollars": best_net_cost,
        "net_mid_cost_dollars": best_net_mid_cost,
        ...
        "bid_ask_drag_bps": best_execution_drag_bps,
        ...
        "display": {
            ...
            "estimated_max_loss_pct": display_max_loss_pct(best_floor_return),
...
def build_zero_cost_target_cap_buffer(
    ...
    # Buy long put at ask.
    long_put_cost = float(long_put["putAskPrice"]) * MULT
    long_put_mid_cost = float(long_put["putMid"]) * MULT
    ...
    for _, short_put in buffer_pool.iterrows():
        ...
        # Sell short put at bid.
        short_put_credit = float(short_put["putBidPrice"]) * MULT
        short_put_mid_credit = float(short_put["putMid"]) * MULT
        ...
        put_spread_mid_cost = long_put_mid_cost - short_put_mid_credit

        for _, call in valid_calls.iterrows():
            call_strike = float(call["strike"])

            # Sell call at bid.
            call_credit = float(call["callBidPrice"]) * MULT
            call_mid_credit = float(call["callMid"]) * MULT

            net_cost = put_spread_cost - call_credit
            net_mid_cost = put_spread_mid_cost - call_mid_credit
            ...
            liq_score, total_volume, total_oi = liquidity_score_legs(
                (long_put, "put"),
                (short_put, "put"),
                (call, "call"),
            )
            ...
            rows.append({
                ...
                "call_mid_credit": call_mid_credit,

                "long_put_strike": long_put_strike,
                "long_put_cost": long_put_cost,
                "long_put_mid_cost": long_put_mid_cost,

                "short_put_strike": short_put_strike,
                "short_put_credit": short_put_credit,
                "short_put_mid_credit": short_put_mid_credit,

                "put_spread_cost": put_spread_cost,
                "put_spread_mid_cost": put_spread_mid_cost,
                "net_cost": net_cost,
                "net_mid_cost": net_mid_cost,
                ...
                "bid_ask_drag_bps": execution_drag_bps,
    ...
    return make_json_safe({
        ...
        "long_put_mid_dollars": float(best["long_put_mid_cost"]),
        "long_put_bid_per_share": float(long_put["putBidPrice"]),
        "long_put_ask_per_share": float(long_put["putAskPrice"]),

        "short_put_credit_dollars": float(best["short_put_credit"]),
        "short_put_mid_dollars": float(best["short_put_mid_credit"]),
        "short_put_bid_per_share": float(short_put["putBidPrice"]),
        "short_put_ask_per_share": float(short_put["putAskPrice"]),

        "put_spread_cost_dollars": float(best["put_spread_cost"]),
        "put_spread_mid_cost_dollars": float(best["put_spread_mid_cost"]),

        "call_credit_dollars": float(best["call_credit"]),
        "call_mid_dollars": float(best["call_mid_credit"]),
        "call_bid_per_share": float(call["callBidPrice"]),
        "call_ask_per_share": float(call["callAskPrice"]),

        "net_cost_dollars": net_cost,
        "net_mid_cost_dollars": float(best["net_mid_cost"]),
        "net_cost_bps": net_cost_bps,
...
def build_defined_outcome_recommendations(
    ...
    payload = {
        ...
        "assumptions": {
            ...
            "cost_note": (
                "Products are priced using executable option markets: "
                "long options at ask and short options at bid. "
                "Strikes with missing or zero bid/ask are excluded."
            ),
        },
        "products": {
...
def build_married_put(
    ...
    # Buy insurance at ask.
    puts["put_cost_dollars"] = puts["putAskPrice"] * MULT
    puts["put_mid_dollars"] = puts["putMid"] * MULT
    puts["bid_ask_drag_dollars"] = puts["put_cost_dollars"] - puts["put_mid_dollars"]
    ...
    liq_score, total_volume, total_oi = liquidity_score_legs((best, "put"))
    ...
    return make_json_safe({
        ...
        "put_mid_dollars": float(best["put_mid_dollars"]),
        "put_cost_per_share": float(best["putAskPrice"]),
...
def build_covered_call(
    ...
    # Sell call at bid.
    calls["call_credit_dollars"] = calls["callBidPrice"] * MULT
    calls["call_mid_dollars"] = calls["callMid"] * MULT
    ...
    calls["bid_ask_drag_dollars"] = calls["call_mid_dollars"] - calls["call_credit_dollars"]
    ...
    liq_score, total_volume, total_oi = liquidity_score_legs((best, "call"))
    ...
    return make_json_safe({
        ...
        "call_mid_dollars": float(best["call_mid_dollars"]),
        "call_bid_per_share": float(best["callBidPrice"]),
        "call_ask_per_share": float(best["callAskPrice"]),
